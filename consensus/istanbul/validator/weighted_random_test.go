...
func TestWeightedCouncil_SubListWithProposer(t *testing.T) {
	...
	view := &istanbul.View{Sequence: big.NewInt(1), Round: big.NewInt(int64(0))}
	for testSubsetLen := 2; testSubsetLen < len(validators); testSubsetLen++ {
		...
		// get committee list and compare with expected committee list
		expectSubList := getExpectSubList(expectIndexOfSubsetLenTest[0:testSubsetLen])
		subListBeforeIstanbulCompatible := valSet.SubList(prevHash, view, false)
		subListAfterIstanbulCompatible := valSet.SubList(prevHash, view, true)
		assert.Equal(t, expectSubList, subListBeforeIstanbulCompatible, "test Subset length: %d(before istanbulCompatible)", testSubsetLen)
		assert.Equal(t, expectSubList, subListAfterIstanbulCompatible, "test subset length: %d(after istanbulCompatible)", testSubsetLen)
	}
	...
	for round := 0; round < 15; round++ {
		...
		view := &istanbul.View{Sequence: big.NewInt(1), Round: big.NewInt(int64(round))}
		// get committee list and compare with expected committee list
		expectSubListBeforeIstanbulCompatible := getExpectSubList(expectIndexOfRoundTestBeforeIstanbulCompatible[round])
		expectSubListAfterIstanbulCompatible := getExpectSubList(expectIndexOfRoundTestAfterIstanbulCompatible[round])
		subListBeforeIstanbulCompatible := valSet.SubList(prevHash, view, false)
		subListAfterIstanbulCompatible := valSet.SubList(prevHash, view, true)
		assert.Equal(t, expectSubListBeforeIstanbulCompatible, subListBeforeIstanbulCompatible, "test round: %d(before istanbulCompatible)", round)
		assert.Equal(t, expectSubListAfterIstanbulCompatible, subListAfterIstanbulCompatible, "test round: %d(after istanbulCompatible)", round)
	}
}

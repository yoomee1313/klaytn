...
func (tx *Transaction) ValidateSender(signer Signer, p AccountKeyPicker, currentBlockNumber uint64, isIstanbul bool) (uint64, error) {
	...
	gasKey, sigValidationErr := accKey.SigValidationGas(currentBlockNumber, tx.GetRoleTypeForValidation(), validSigNum, isIstanbul)

	if sigValidationErr != nil {
		return 0, sigValidationErr
	}
...
func (tx *Transaction) ValidateFeePayer(signer Signer, p AccountKeyPicker, currentBlockNumber uint64, isIstanbul bool) (uint64, error) {
	...
	gasKey, sigValidationErr := accKey.SigValidationGas(currentBlockNumber, accountkey.RoleFeePayer, validSigNum, isIstanbul)

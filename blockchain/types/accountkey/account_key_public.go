...
func (a *AccountKeyPublic) Validate(r RoleType, recoveredKeys []*ecdsa.PublicKey, from common.Address, isIstanbul bool) (bool, int) {
	// AccountKeyPublic has only one public key.
	if len(recoveredKeys) != 1 {
		return false, 0
	}

	return a.PublicKeySerializable.Equal((*PublicKeySerializable)(recoveredKeys[0])), 1
}
...
func (a *AccountKeyPublic) SigValidationGas(currentBlockNumber uint64, r RoleType, validSigNum int, isIstanbul bool) (uint64, error) {
